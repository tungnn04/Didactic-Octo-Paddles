module.exports = (db) => {
  ...
  router.post("/login", async (req, res) => {
    try {
      ...
      if (!user) {
        return res.status(400).send(response("Invalid username or password"));
      }
  ...
  router.post("/add-to-cart/:item", AuthMiddleware, async (req, res) => {
    ...
    try {
      if (!item) {
        return res.status(400).send(response("Item needs to be specified"));
      }
      ...
      if (cart.productIds.includes(item)) {
        return res.status(400).send(response("Product already in cart"));
      }
  ...
  router.get("/admin", AdminMiddleware, async (req, res) => {
    try {
      ...
      // PATCH:
      // Chỉ truyền dữ liệu thuần sang view.
      // Không dùng jsrender.templates(usernames).render()
      //   res.render("admin", {
      //     users: usernames,
      //   });
    } catch (error) {
